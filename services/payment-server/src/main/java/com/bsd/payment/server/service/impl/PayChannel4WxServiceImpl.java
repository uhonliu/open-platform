...
public class PayChannel4WxServiceImpl extends BaseService implements IPayChannel4WxService {
            ...
            try {
                ...
                switch (tradeType) {
                    ...
                    case PayConstant.WxConstant.TRADE_TYPE_APP: {
                        ...
                        // 此map用于客户端与微信服务器交互
                        payInfo.put("sign", SignUtils.createSign(configMap, WxPayConstants.SignType.MD5, wxPayConfig.getMchKey(), null));
                    ...
                    case PayConstant.WxConstant.TRADE_TYPE_JSAPI: {
                        Map<String, String> payInfo = new HashMap<>();
                        String timestamp = String.valueOf(System.currentTimeMillis() / 1000);
                        String nonceStr = String.valueOf(System.currentTimeMillis());
                        ...
                        payInfo.put("paySign", SignUtils.createSign(payInfo, WxPayConstants.SignType.MD5, wxPayConfig.getMchKey(), null));
                        map.put("payParams", payInfo);
                        break;
                    }
    ...
    public Map doWxRefundReq(String jsonParam) {
        ...
        try {
            ...
            String channelId = refundOrder.getChannelCode();
    ...
    WxPayUnifiedOrderRequest buildUnifiedOrderRequest(PayOrder payOrder, WxPayConfig wxPayConfig) {
        ...
        String openId = null;
        if (tradeType.equals(PayConstant.WxConstant.TRADE_TYPE_JSAPI)) {
