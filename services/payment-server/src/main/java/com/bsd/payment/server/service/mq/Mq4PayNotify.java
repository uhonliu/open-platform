 ...
 */
public abstract class Mq4PayNotify {
    @Resource
    ...
    public void receive(String msg) {
        ...
        try {
            ...
            if ("success".equalsIgnoreCase(notifyResult.trim())) {
                //_log.info("{} notify success, url:{}", _notifyInfo.getBusiId(), respUrl);
                //修改订单表
                try {
                    int result = baseService.baseUpdateStatus4Complete(orderId);
                ...
                // 修改通知次数
                try {
                    int result = baseService.baseUpdateNotify(orderId, (byte) 1);
                ...
                // 修改通知次数
                try {
                    int result = baseService.baseUpdateNotify(orderId, (byte) cnt);
                    _log.info("修改payOrderId={},通知业务系统次数->{}", orderId, result == 1 ? "成功" : "失败");
                } catch (Exception e) {
                    _log.error(e, "修改通知次数异常");
                }
