...
import com.bsd.payment.server.service.Notify4Pay;
 ...
 */
@Service
public class NotifyPayServiceImpl extends Notify4Pay implements INotifyPayService {
    ...
    public boolean verifyAliPayParams(Map<String, Object> payContext) {
        ...
        if (payChannel == null) {
            _log.error("Can't found payChannel form db. mchId={} channelCode={}, ", payOrderId, mchId, channelCode);
            payContext.put("retMsg", "Can't found payChannel");
            return false;
        }
        boolean verify_result = false;
        try {
            verify_result = AlipaySignature.rsaCheckV1(params, alipayProperties.init(payChannel.getParam()).getPublicKey(), AlipayProperties.CHARSET, "RSA2");
