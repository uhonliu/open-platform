...
public class DeveloperServiceImpl implements DeveloperService {
    ...
    public JsSdkSignDTO makeWxJsSdkSign(String url) {
        //获取公众号配置信息
        OpenOAuth2ClientDetails config = getWxGzhConfig();
        //获取微信jsapi_ticket
     ...
     * @return
     */
    private OpenOAuth2ClientDetails getWxGzhConfig() {
        OpenOAuth2ClientDetails config = openOAuth2ClientProperties.getOauth2().get("wechat_gzh");
        if (config == null) {
            throw new OpenAlertException("公众号配置信息尚未初始化,请稍后重试");
        }
        //第三方用户唯一凭证
        String appid = config.getClientId();
        if (StringUtils.isEmpty(appid)) {
            throw new OpenAlertException("公众号appid未配置");
        }
        //第三方用户唯一凭证密钥
        String appsecret = config.getClientSecret();
        if (StringUtils.isEmpty(appsecret)) {
            throw new OpenAlertException("公众号appsecret未配置");
        }
        return config;
    }
